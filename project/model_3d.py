...
class SmallUNet3D(nn.Module):
    """Small 3D U-Net style model for volumetric heatmap regression."""
    ...
    def _center_crop_to_match(source: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if (d_s, h_s, w_s) == (d_t, h_t, w_t):
            return source
        if d_s < d_t or h_s < h_t or w_s < w_t:
            raise ValueError(f"Cannot crop source {source.shape} to larger target {target.shape}")

        z0 = (d_s - d_t) // 2
        y0 = (h_s - h_t) // 2
        x0 = (w_s - w_t) // 2
        return source[:, :, z0 : z0 + d_t, y0 : y0 + h_t, x0 : x0 + w_t]

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        in_d, in_h, in_w = x.shape[-3:]
        ...
        if x.shape[-3:] != (in_d, in_h, in_w):
            x = F.interpolate(x, size=(in_d, in_h, in_w), mode="trilinear", align_corners=False)
        return x
